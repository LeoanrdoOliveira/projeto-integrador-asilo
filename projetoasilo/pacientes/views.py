...
def gerar_receita(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            p.setFont("Helvetica-Bold", 12)
            p.drawString(2*cm, height - 7*cm, "Data:")
            p.setFont("Helvetica", 12)
            p.drawString(7*cm, height - 7*cm, form.cleaned_data['data_receita'].strftime('%d/%m/%Y'))

            # Seção de prescrição
            p.setFillColor(colors.darkblue)
            p.setFont("Helvetica-Bold", 14)
            p.drawString(2*cm, height - 9*cm, "Prescrição")
            p.setLineWidth(0.5)
            p.setStrokeColor(colors.darkgrey)
            p.line(2*cm, height - 9.3*cm, width - 2*cm, height - 9.3*cm)
